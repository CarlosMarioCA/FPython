...
def createLayout():
    layout = [[sg.Text("TITTLE", auto_size_text = True),],
            [sg.Button("", key="-0-", size=button_size), sg.Button("", key="-1-", size=button_size), sg.Button("", key="-2-", size=button_size)],
...
def createWindow():
    layout = createLayout()
    window = sg.Window("TRIKI 1.0", layout, margins=(10, 10), element_justification='c')
    return window
